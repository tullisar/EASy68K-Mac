 ...
 *              Removed includes not relevant to Mac OS X Port
 *              
 ************************************************************************/
        ...
        if (isspace(*op) || !*op)
            if (!*label) {
                ...
                if (pass2 && listFlag && *errorPtr < MINOR) {
                    // TODO: (WORKAROUND) Ensure cast to unsigned long isn't a bad idea at a later point
                    sprintf(listPtr, "=%08lX ", (unsigned long)value);
                    listPtr += 10;
                }
            }
            else
    ...
    if (*errorPtr < ERRORN)          // ck 4-18-03
        if (isspace(*op) || !*op)
            if (!*label) {
                ...
                if (pass2 & listFlag) {
                    // TODO: (WORKAROUND) Ensure cast to unsigned long isn't bad idea
                    sprintf(listPtr, "=%08lX ", (unsigned long)value);
                    listPtr += 10;
                }
            }
    ...
    do {
        ...
        notString:                              // process non strings
            ...
            if (pass2)
                output(outVal, size);
            loc += size;
            if (size == BYTE_SIZE_M && (outVal < -128 || outVal > 255)) {
...
int include(int size, char *label, char *fileName, int *errorPtr)
{
    ...
    // Strip whitespace from end of file name
    dst--;
    ...
    *dst = '\0';
    ...
    // If include directive should be listed
    if (pass2 && listFlag) {
        listLine(line);
    }
    try {
        ...
        // TODO: (GUI) Determine if included file is already open in another editor window
    ...
    *dst = '\0';
    ...
    try {
        ...
        while(fread(&dataByte, 1, 1, incFile) && !feof(incFile)) {
            ...
            if (pass2) {
                if (objFlag)
                    outputObj(loc, dataByte, BYTE_SIZE_M);
            }
