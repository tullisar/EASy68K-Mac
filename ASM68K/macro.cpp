 ...
 *             http://www.wowgwep.com/EASy68K.htm
       ...
       Modified: 2011-01-28
                 ...
                 * Changed itoa() to use sprintf() as itoa() is not 
                   supported by the compiler under Mac OS X
...
int macro(int size, char *label, char *op, int *errorPtr)
{
    ...
    while(fgets(line, 256, inFile)) {
        if (pass == 0)
            fprintf(tmpFile,"%s",line);           // write macro line to tmpFile
        lineNum++;
        tokenize(line, " \t\n", token, tokens);
...
int asmMacro(int size, char *label, char *arg, int *errorPtr)
{
    ...
    sprintf(labelNumA, "%d", labelNum);     // convert labelNum to string (using sprintf, RBS)
